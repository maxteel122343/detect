  ...
  return (
    <div className="flex grow flex-col gap-3">
      ...
      <div className="w-full flex flex-col">
        ...
        ) : (detectType as string) === 'Segmentation masks' ? (
          <div className="flex flex-col gap-2">
            <div>{(prompts as any)[detectType]?.[0]}</div>
            <textarea
              className="w-full bg-[var(--input-color)] rounded-lg resize-none p-4"
              placeholder="What to segment?"
              rows={1}
              value={(prompts as any)[detectType]?.[1] ?? ''}
              onChange={(e) => {
                const value = e.target.value;
                const newPromptsState = {...prompts};
                if (!(newPromptsState as any)[detectType])
                  (newPromptsState as any)[detectType] = ['', '', ''];
                (newPromptsState as any)[detectType][1] = value;
